...
function watchTheme() {
  if (darkMq || !window.matchMedia) return;
  darkMq = window.matchMedia("(prefers-color-scheme: dark)");
  const handler = () => {
    if (!map || !tileLayer) return;
    map.removeLayer(tileLayer);
    tileLayer = makeTileLayer(currentTheme()).addTo(map);
  };
  // Modern API + legacy fallback
  if (darkMq.addEventListener) darkMq.addEventListener("change", handler);
  else if (darkMq.addListener) darkMq.addListener(handler);
}
