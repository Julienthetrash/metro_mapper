...
function paintDetail(lineId) {
  ...
  const stations = state.stations
    ...
    .sort((a, b) => {
      // Use the geographic order computed at data-generation time (heuristic
      // nearest-neighbor from one terminus). Falls back to alpha sort if the
      // field is missing for any reason.
      const oa = a.orderByLine?.[lineId];
      const ob = b.orderByLine?.[lineId];
